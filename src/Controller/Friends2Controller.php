...
use App\Form\Friends2Type;

class Friends2Controller extends Controller
{
	public function index(Request $request)
	{
		...
		$form = $this->createForm(Friends2Type::class);
...
#			$party = $request->request->get('selekt2');

			$party_id = $request->request->all()['friends2']['selekt'];
			#$party = $request->request->all();

			
			$party = $this->getDoctrine()->getRepository(User::class)->find($party_id);
	
			#$party = $form->get('accounts')->getData();
			#$party = $form->get("selekt")->getData();
			#$party = $form->get('srch')->getData();
			...
			return $this->redirectToRoute('friends2');
...
#getuser?
		...
		return $this->render('port119/friends2.html.twig', [ 'username' => $username, 'form' => $form->createView(), 'frepotoconfirm' => $frepotoconfirm, 'freporequested' => $freporequested, 'frepo' => $frepo, 'av' => $av, 'cav' => $cav, 'rav' => $rav, 'host' => $hurl ]);
